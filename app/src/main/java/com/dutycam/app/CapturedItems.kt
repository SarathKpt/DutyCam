...
import com.dutycam.app.CamConfig.SettingValues
...
object CapturedItems {
    ...
    fun parseCapturedItem(fileName: String, uri: Uri): CapturedItem? {
        val dcIndex = fileName.indexOf(".DC_")
        if (dcIndex != -1) {
            val type = if (fileName.endsWith(".jpg", true) || fileName.endsWith(".jpeg", true)) {
                ITEM_TYPE_IMAGE
            ...
            val startOfDate = dcIndex + 4 // after ".DC_"
            val endOfDate = fileName.lastIndexOf('.')
            if (endOfDate <= startOfDate) return null

            val dateString = fileName.substring(startOfDate, endOfDate)
            return CapturedItem(type, dateString, uri)
        }
